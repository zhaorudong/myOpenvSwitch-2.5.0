                ...
                const struct cls_conjunction conj[], size_t n)
{
   ...
   // *CONST_CAST(int *, &cls_match->first_DPID) = rule->first_DPID;
...
static inline void
cls_rule_init2__(struct cls_rule *rule, unsigned int priority,unsigned int first_DPID)
{
    rculist_init(&rule->node);
    *CONST_CAST(int *, &rule->priority) = priority;
    *CONST_CAST(int *, &rule->first_DPID) = first_DPID;
    rule->cls_match = NULL;
    //WriteC_lib(__FILE__,__func__,first_DPID);
}
...
void
cls_rule_init2(struct cls_rule *rule, const struct match *match, int priority,int first_DPID)
{
    //cls_rule_init__(rule, priority);
    cls_rule_init2__(rule, priority,first_DPID);
    minimatch_init(CONST_CAST(struct minimatch *, &rule->match), match);
}
...
cls_rule_clone(struct cls_rule *dst, const struct cls_rule *src)
{
    ...
    cls_rule_init2__(dst, src->priority,src->first_DPID);
