...
/* ## ----------------- ## */
    ...
    /* OF1.0(28): struct nx_action_handle_example. */
    OFPAT_RAW10_HANDLE_EXAMPLE,
 
    /* OF1.1(28): struct nx_action_handle_example. */
    OFPAT_RAW11_HANDLE_EXAMPLE,
 
    /* OF1.2-1.4(28): struct nx_action_handle_example. */
...
//=========ZRD=====================
struct nx_action_handle_example {
    ovs_be16 type;                  /* OFPAT_VENDOR. */
    ovs_be16 len;                   /* Length is 16. */
    ovs_be32 vendor;                /* NX_VENDOR_ID. */
    ovs_be16 subtype;               /* NXAST_CONTROLLER. */
...
OFP_ASSERT(sizeof(struct nx_action_handle_example) == 16);

static enum ofperr
decode_OFPAT_RAW10_HANDLE_EXAMPLE(const struct nx_action_handle_example *nac,
                            enum ofp_version ofp_version OVS_UNUSED,
                            struct ofpbuf *out)
{
    WriteC2(__func__);
    struct ofpact_handle_example *oc;

    oc = ofpact_put_HANDLE_EXAMPLE(out);
    oc->max_len = ntohs(nac->max_len);
    oc->controller_id = ntohs(nac->controller_id);
    oc->reason = nac->reason;
    return 0;
...
static enum ofperr
decode_OFPAT_RAW11_HANDLE_EXAMPLE(const struct nx_action_handle_example *nac,
                            enum ofp_version ofp_version OVS_UNUSED,
                            struct ofpbuf *out)
{
    WriteC2(__func__);
...
static enum ofperr
decode_OFPAT_RAW12_HANDLE_EXAMPLE(const struct nx_action_handle_example *nac,
                            enum ofp_version ofp_version OVS_UNUSED,
                            struct ofpbuf *out)
{
    WriteC2(__func__);
...
static void
encode_HANDLE_EXAMPLE(const struct ofpact_handle_example *controller,
                  enum ofp_version ofp_version OVS_UNUSED,
                  struct ofpbuf *out)
{
    WriteC2(__func__);
    struct nx_action_handle_example *nac;

     if (ofp_version == OFP10_VERSION) {
        nac=put_OFPAT10_HANDLE_EXAMPLE(out);
    } else  if (ofp_version == OFP11_VERSION) {
        nac=put_OFPAT11_HANDLE_EXAMPLE(out);
    } else {
        nac=put_OFPAT12_HANDLE_EXAMPLE(out);
    }
                  ...
                  enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    ...
    if (!arg[0]) {
        ...
        struct ofpact_handle_example *controller;

        controller = ofpact_put_HANDLE_EXAMPLE(ofpacts);
        controller->max_len = max_len;
        controller->reason = reason;
        controller->controller_id = controller_id;
...
format_HANDLE_EXAMPLE(const struct ofpact_handle_example *a, struct ds *s)
{
    WriteC2(__func__);
    if (a->reason == OFPR_ACTION && a->controller_id == 0) {
        ds_put_format(s, "HANDLE_EXAMPLE:%"PRIu16, a->max_len);
    } else {
        enum ofp_packet_in_reason reason = a->reason;

        ds_put_cstr(s, "handle_example(");
        if (reason != OFPR_ACTION) {
            char reasonbuf[OFPUTIL_PACKET_IN_REASON_BUFSIZE];
                       ...
                       struct ofpbuf *ofpacts)
{
    ...
    //learn->first_DPID= ntohs(nal->first_DPID);
    //WriteC_lib(__FILE__,__func__,learn->first_DPID);
             ...
             enum ofp_version ofp_version OVS_UNUSED, struct ofpbuf *out)
{
    ...
    //nal->first_DPID = htons(learn->first_DPID);
    memset(nal->pad2, 0, sizeof nal->pad2);
    //WriteC_lib(__FILE__,__func__,nal->first_DPID);
