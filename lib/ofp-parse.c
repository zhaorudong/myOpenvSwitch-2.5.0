                ...
                enum ofputil_protocol *usable_protocols)
{
    ...
    *usable_protocols = OFPUTIL_P_ANY;
    ...
    case OFPFC_ADD:
        fields = F_ACTIONS | F_TIMEOUT | F_PRIORITY | F_FLAGS | F_IMPORTANCE|F_FIRST_DPID;
        break;
    ...
    case OFPFC_DELETE_STRICT:
        fields = F_OUT_PORT | F_PRIORITY|F_FIRST_DPID;
        break;

    case OFPFC_MODIFY:
        fields = F_ACTIONS | F_TIMEOUT | F_PRIORITY | F_FLAGS|F_FIRST_DPID;
        break;
    ...
    fm->first_DPID= OFP_DEFAULT_PRIORITY;
    //WriteC_lib(__FILE__,__func__,__LINE__);
    fm->cookie = htonll(0);
    ...
    while (ofputil_parse_key_value(&string, &name, &value)) {
		...
		//WriteC_lib(__FILE__,__func__,__LINE__);
            } else if (fields & F_FIRST_DPID && !strcmp(name, "first_DPID")) {
                uint16_t first_DPID = 0;
		
                error = str_to_u16(value, name, &first_DPID);
                fm->first_DPID= first_DPID;
		//WriteC_lib(__FILE__,__func__,__LINE__);
            } 
