...
void
match_format2(const struct match *match, struct ds *s, int priority,int first_DPID)
{
    const struct flow_wildcards *wc = &match->wc;
    size_t start_len = s->length;
    const struct flow *f = &match->flow;
  ...
  if (first_DPID != OFP_DEFAULT_PRIORITY) {
  	
        ds_put_format(s, "first_DPID=%d,", first_DPID);
    }
