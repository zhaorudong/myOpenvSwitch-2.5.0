                        ...
                        ofp_port_t max_port, uint8_t max_table)
{
    ...
    //WriteC_lib(__FILE__,__func__,__LINE__);
	...
	fm->first_DPID=ntohs(ofm->first_DPID);
	//WriteC_lib(__FILE__,__func__,__LINE__);
        ...
        if (raw == OFPRAW_OFPT10_FLOW_MOD) {
            ...
            fm->priority = (ofm->match.wildcards & htonl(OFPFW10_ALL)
                            ? ntohs(ofm->priority)
                            : UINT16_MAX);
	 fm->first_DPID= (ofm->match.wildcards & htonl(OFPFW10_ALL)
                            ? ntohs(ofm->first_DPID)
                            : UINT16_MAX);
		//WriteC_lib(__FILE__,__func__,fm->first_DPID);
            /* Translate the message. */
	    ...
	    fm->first_DPID=ntohs(nfm->first_DPID);
	    //WriteC_lib(__FILE__,__func__,fm->first_DPID);
                        ...
                        enum ofputil_protocol protocol)
{
	...
	ofm->first_DPID = htons(fm->first_DPID);
	//WriteC_lib(__FILE__,__func__,__LINE__);
    ...
    case OFPUTIL_P_OF10_STD_TID: {
        ...
        ofm->priority = htons(fm->priority);
	ofm->first_DPID=htons(fm->first_DPID);
	//WriteC_lib(__FILE__,__func__,ofm->first_DPID);
	memset(ofm->pad, 0, sizeof ofm->pad);
        ofm->buffer_id = htonl(fm->buffer_id);
	...
	nfm->first_DPID= htons(fm->first_DPID);
	//WriteC_lib(__FILE__,__func__,nfm->first_DPID);
	memset(nfm->pad2, 0, sizeof nfm->pad2);
                                ...
                                struct ofpbuf *ofpacts)
{
	...
	fs->first_DPID=ntohs(ofs->first_DPID);
	//WriteC_lib(__FILE__,__func__,fs->first_DPID);
	...
	fs->first_DPID= ntohs(nfs->first_DPID);
	//WriteC_lib(__FILE__,__func__,fs->first_DPID);
                                ...
                                struct ovs_list *replies)
{
	...
	ofs->first_DPID= htons(fs->first_DPID);
	//WriteC_lib(__FILE__,__func__,ofs->first_DPID);
	memset(ofs->pad3, 0, sizeof ofs->pad3);
	...
	nfs->first_DPID=htons(fs->first_DPID);
	//WriteC_lib(__FILE__,__func__,nfs->first_DPID);
                           ...
                           struct ofpbuf *msg, struct ofpbuf *ofpacts)
{
	...
	update->first_DPID= ntohs(nfuf->first_DPID);
	//WriteC_lib(__FILE__,__func__,update->first_DPID);
                           ...
                           struct ovs_list *replies)
{
	...
	nfuf->first_DPID=htons(update->first_DPID);
	//WriteC_lib(__FILE__,__func__,__LINE__);
