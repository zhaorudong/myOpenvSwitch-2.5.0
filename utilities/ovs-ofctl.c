...
compare_flows(const void *afs_, const void *bfs_)
{
	...
	printf("fileName:%s func:%s lineNum:%d",__FILE__,__func__,__LINE__);
                 ...
                 size_t n_fms, enum ofputil_protocol usable_protocols)
{
  ...
  //WriteC_ofproto(__FILE__,__func__,fms->first_DPID);
    vconn_close(vconn);
}
...
static void
fte_insert2(struct flow_tables *tables, const struct match *match,
           int priority,int first_DPID, struct fte_version *version, int index)
{
    struct classifier *cls = &tables->tables[version->table_id];
    struct fte *old, *fte;
    ...
    fte = xzalloc(sizeof *fte);
    //cls_rule_init(&fte->rule, match, priority);
    cls_rule_init2(&fte->rule, match, priority,first_DPID);
    fte->versions[index] = version;
                     ...
                     int index)
{
	...
	//WriteC_ofproto(__FILE__,__func__,fm.first_DPID);
                       ...
                       struct flow_tables *tables, int index)
{
	...
	fte_insert2(tables, &fs.match, fs.priority,  fs.first_DPID,version, index);
	//WriteC_ofproto(__FILE__,__func__,fs.first_DPID);
    }
    flow_tables_publish(tables);
    ofpbuf_uninit(&ofpacts);
}
                  ...
                  enum ofputil_protocol protocol, struct ovs_list *packets)
{
   ...
   //WriteC_lib(__FILE__,__func__,__LINE__);
    ...
    fm.first_DPID= fte->rule.first_DPID;
    fm.cookie = htonll(0);
